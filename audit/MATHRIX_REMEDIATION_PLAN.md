...
9. **[ DONE, one characterised limitation ] DEF-004 — authored cursor targets instead of fuzzy word-matching.** `focusTargetIds` is plumbed end-to-end and resolved via `data-teacher-id`. It is *derived* from the `noteKeys`/`cellKeys`/`carryKeys` a step already declares rather than emitted separately by every builder — those keys **are** the pen order, so this keeps one source of truth; a parallel list would drift the first time a builder changed its reveal and forgot the copy, and a drifted cursor path fails invisibly because it still points at a real cell. An explicit `focusTargetIds` overrides.

    Two timing bugs surfaced on the way, the second only because playback was sampled live: exact in-order alignment replaces the proportional guess, and — since a step's anchors are individual digits while narration says numbers whole ("Write 36 on top and 15 underneath" never speaks 3, 6, 1 or 5; one step said "10" as "a zero") — `pinAnchorsToNarration` pins what *is* spoken and interpolates the rest instead of discarding every good pin when one anchor is unmatchable.

    The old check (`scripts/audit-cursor-sync.ts`) asked only whether the cursor was inside **some** teaching target. Every cell of a column method is a target, so it passed while the cursor sat on the wrong digit — precisely the gap this defect lived in. `scripts/audit-cursor-semantics.ts` replaces it and separately verifies that the authored path *fired* (not that inference happened to agree). Live on the shipped code across column addition, column subtraction (the borrow path, where `noteKeys` is the only anchor source) and long multiplication: **1516/1516 frames on the authored path, 11/13 spoken anchors hit, median arrival 0ms.**

    Remaining limitation, deliberate and test-locked: narration that recites anchors *against* pen order ("5 × 6" when the pen writes the 6 cell first) cannot be fully satisfied by a monotonic cursor, and a hand that jumps backwards reads as a glitch. Both residual failures are that one class.
