/**
 * Runtime systems check for every KS2 maths topic page in both Learn and Guided.
 * Usage: npx tsx scripts/audit-ks2-maths-runtime.ts [baseUrl]
 */
...
async function main() {
  ...
  for (const topic of topics) {
    ...
    if (!context) continue;
    const preferred = PREFERRED_SKILL[topic.id];
    const skill = context.topic.subtopics.includes(preferred)
      ? preferred
      : context.topic.subtopics[0] || context.topic.name;
    jobs.push(
      { topicId: topic.id, topic: context.topic.name, skill, kind: "lesson" },
      { topicId: topic.id, topic: context.topic.name, skill, kind: "guided" },
    );
  }
